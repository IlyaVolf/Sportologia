...
import com.thesis.sportologia.core.entities.toPosition
...
import com.thesis.sportologia.data.events.entities.EventFirestoreEntity
...
class FirestoreEventsDataSource @Inject constructor() : EventsDataSource {
    ...
    override suspend fun getPagedEvents(
        ...
        val query = database.collection(EVENTS_PATH)

        /*database.collection(EVENTS_PATH).get().addOnSuccessListener { snap ->
            snap.documents.forEach { doc ->
                database.collection(EVENTS_PATH).document(doc.id).update("tokens", FieldValue.delete())
            }
        }

        database.collection(EVENTS_PATH).get().addOnSuccessListener { snap ->
            snap.documents.forEach { doc ->
                database.collection(EVENTS_PATH).document(doc.id).update(
    ...
    override suspend fun getEvent(eventId: Long, userId: Long): EventDataEntity {
        val eventDocument = database.collection(EVENTS_PATH)
            .document(eventId)
            .get()
            .addOnFailureListener { e ->
                throw Exception(e)
    ...
    override suspend fun createEvent(organizer: UserDataEntity, createEventDataEntity: CreateEventDataEntity) {
        val photosFirestore = mutableListOf<Uri>()
        createEventDataEntity.photosUrls.forEach {
        ...
        val eventFirestoreEntity = hashMapOf(
            ...
            "description" to createEventDataEntity.description,
            "organizerId" to organizer.id,
            "userType" to organizer.userType,
            "dateFrom" to createEventDataEntity.dateFrom,
            "dateTo" to createEventDataEntity.dateTo,
            "position" to GeoPoint(
                createEventDataEntity.position.latitude,
                createEventDataEntity.position.longitude
            ),
            "price" to createEventDataEntity.price,
            "currency" to createEventDataEntity.currency,
            "categories" to createEventDataEntity.categories,
            "likesCount" to 0,
        ...
        val documentRef = database.collection(EVENTS_PATH).document()
        ...
        documentRef
            .update(
                hashMapOf(
                    ...
                    "datePlusId" to "${createEventDataEntity.dateFrom}${documentRef.id}",
                    "tokens" to organizer.name.split(" ").filter { it.isNotBlank() }
                        .map {
                            it.lowercase(
                                Locale.getDefault()
                            )
    ...
    suspend fun a222() {
        val a = database.collection(EVENTS_PATH)
            .whereIn("organizerId", listOf("i_chiesov"))
            .get()
            .await()

        a.documents.forEach {
            database.collection(EVENTS_PATH).document(it.id).update(hashMapOf<String, Any?>(
                "description" to "Очередная встреча любителей лыж состоится 1 февраля на стадионе НГУ",
                "dateFrom" to 1675242000000L,
                "dateTo" to null,
                "price" to 0,
                "datePlusId" to "1675242000000L${it.id}",
                "position" to GeoPoint(
                    54.847534, 83.096105
                ),
            )).await()
